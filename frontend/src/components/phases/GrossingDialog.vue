<template>
    <v-dialog :model-value="visible" @update:model-value="emit('update:visible', $event)" max-width="600">
        <v-card>
        <v-card-title class="text-h6">Grossing - {{ order.specimen_id }}</v-card-title>
        <v-card-text>
            <!-- Your Grossing Phase Form Here -->
            <p>Perform Grossing phase actions for order ID: {{ order.specimen_id }}</p>
        </v-card-text>
        <v-card-actions>
            <v-spacer></v-spacer>
...
<script setup>
    ...
    function save() {
        console.log('Saving Grossing phase for:', props.order)
        emit('update:visible', false)
    }
</script>
