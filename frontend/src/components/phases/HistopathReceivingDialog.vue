<template>
    <v-dialog :model-value="visible" @update:model-value="emit('update:visible', $event)" max-width="600">
        <v-card>
        <v-card-title class="text-h6">Histopath Receiving - {{ order.specimen_id }}</v-card-title>
        <v-card-text>
            <!-- Your Receiving Phase Form Here -->
            <p>Perform Histopath Receiving phase actions for order ID: {{ order.specimen_id }}</p>
        </v-card-text>
        <v-card-actions>
            <v-spacer></v-spacer>
